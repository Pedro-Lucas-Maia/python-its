...
export default function LessonClient({ moduleId }: { moduleId: string }) {
  ...
  const [feedback, setFeedback] = useState<{ isCorrect: boolean; isCompleted: boolean } | null>(null);

  // Fetch or Generate Lesson using TanStack Query
  ...
  const handleNext = () => {
    if (feedback?.isCompleted) {
      ...
      // Invalida a query para forçar o AI a gerar uma nova pergunta
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] gap-4">
        ...
        <p className="text-neutral-500 font-medium">
          {isLoading ? "Sua IA tutora está preparando o conteúdo..." : "Analisando sua resposta e gerando a próxima etapa..."}
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-red-50 dark:bg-red-950/30 text-red-700 dark:text-red-400 p-6 rounded-xl border border-red-100 dark:border-red-900/50 text-center flex flex-col gap-4 max-w-lg mx-auto mt-12">
        ...
        <div>
          ...
          <p className="text-sm opacity-90">Ocorreu um erro ao chamar a Inteligência Artificial. Verifique se você configurou a chave <code>GOOGLE_GENERATIVE_AI_API_KEY</code> corretamente no arquivo <code>.env.local</code>.</p>
        </div>
        <Link href="/dashboard" className="px-6 py-2 bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-300 rounded-full font-medium self-center mt-2 hover:opacity-80 transition-opacity">
          Voltar
        </Link>
      </div>
    );
  ...
  return (
    <div className="max-w-2xl mx-auto w-full flex flex-col gap-8 duration-500 pb-20">
      <Link href="/dashboard" className="text-sm text-neutral-500 hover:text-neutral-900 dark:hover:text-white transition-colors mb-2 inline-flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-neutral-900 p-6 md:p-8 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-[0_4px_24px_rgba(0,0,0,0.02)] transition-all">
        ...
        <div className="bg-neutral-50 dark:bg-neutral-950 p-6 rounded-2xl border border-neutral-100 dark:border-neutral-800">
          ...
          <div className="flex flex-col gap-3">
            {content.alternativas.map((alt: string, index: number) => {
              ...
              let buttonStyle = "bg-white dark:bg-neutral-900 border-neutral-200 dark:border-neutral-700 text-neutral-700 dark:text-neutral-300 hover:border-neutral-400 dark:hover:border-neutral-500 hover:bg-neutral-50 dark:hover:bg-neutral-800";
              
              if (isSelected && !hasAnswered) {
                buttonStyle = "bg-neutral-900 dark:bg-white border-neutral-900 dark:border-white text-white dark:text-neutral-900 ring-2 ring-neutral-900/20";
        ...
        {/* Floating action bar when option is selected */}
        {selectedOption !== null && !hasAnswered && (
          <div className="mt-8 flex justify-end">
            <button
              onClick={() => submitMutation.mutate(selectedOption)}
              disabled={submitMutation.isPending}
              className="bg-neutral-900 dark:bg-white text-white dark:text-neutral-900 px-6 py-3 rounded-full font-medium hover:scale-105 active:scale-95 transition-all shadow-lg flex items-center gap-2"
            >
              Confirmar resposta
            </button>
          </div>
        ...
        {/* Feedback Section */}
        {feedback !== null && (
          <div className={`mt-8 p-6 rounded-2xl flex flex-col sm:flex-row items-center gap-6 justify-between ${
            feedback.isCorrect ? 'bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-900/50' : 'bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900/50'
          }`}>
            <div className="flex items-center gap-4 text-center sm:text-left">
              ...
              <div>
                <h4 className={`font-bold ${feedback.isCorrect ? 'text-emerald-800 dark:text-emerald-400' : 'text-amber-800 dark:text-amber-400'}`}>
                  {feedback.isCorrect ? 'Excelente! Resposta correta.' : 'Ops! Não foi dessa vez.'}
                </h4>
                <p className={`text-sm ${feedback.isCorrect ? 'text-emerald-600 dark:text-emerald-500' : 'text-amber-600 dark:text-amber-500'}`}>
                  {feedback.isCorrect 
                    ? (feedback.isCompleted ? 'Você dominou este módulo completamente!' : 'Você ganhou experiência. Vamos para o próximo passo.') 
            ...
            <button
              ...
              className={`shrink-0 px-6 py-3 rounded-full font-medium flex items-center gap-2 hover:opacity-90 active:scale-95 transition-all shadow-sm ${
                feedback.isCorrect ? 'bg-emerald-600 text-white' : 'bg-amber-600 text-white'
              }`}
            >
